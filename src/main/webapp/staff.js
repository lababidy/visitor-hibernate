...
// Model for Staff entity
    models.Staff = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/webresources/org.lababidy.visitor.hibernate.staff/",
        idAttribute: 'sid',
        defaults: {
            sname: "",
            spassword: ""
        ...
        toViewJson: function () {
            ...
            result.displayName = this.get('sid');
            return result;
        },
        isNew: function () {
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/webresources/org.lababidy.visitor.hibernate.staff/';
            }
    ...
    // Collection class for Staff entities
    models.StaffCollection = Backbone.Collection.extend({
        model: models.Staff,
        url: "http://localhost:8080/webresources/org.lababidy.visitor.hibernate.staff/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.StaffCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-staff-list-itemis template identifier for item
                        templateName: '#tpl-staff-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.staff = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.staff,
                    // tpl-staff-details is template identifier for chosen model element
                    templateName: '#tpl-staff-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Staff();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-staff-details is a template identifier for chosen model element
                templateName: '#tpl-staff-details',
